...
/* void Fase::InicializaRec(RenderWindow* window, float deltaTime) // não utilizar
{
	this->window = window;
	this->deltaTime = deltaTime;
	jog1Texture.loadFromFile("kit_from_firefox.png");
...
void Fase::salvarJogada()
{
	...
	else
	{
		...
		//gravar entidades
		Lista<Entidade>::Elemento<Entidade>* pAux = listaEnt.getPrim()->getProx();
		while (pAux != NULL)
		{
...
void Fase::ExecutarRec(int i)
{
	...
	while (window->isOpen())
	{
		...
		if (!pause)
		{
			...
			if (multiplayer)
			{
				if ((jog1.getPorta() && jog2.getPorta()) || jog2.esgotou()) //alterar se necessário
					break;
			}
			else
			{
